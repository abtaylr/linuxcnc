...
static void update_status(void)
{
    ...
    emcmotStatus->external_offsets_applied = 0;
    for (axis_num = 0; axis_num < EMCMOT_MAX_AXIS; axis_num++) {
	/* point to axis data */
	axis = &axes[axis_num];
	...
	if (fabs(axis->ext_offset_tp.curr_pos) > EOFFSET_EPSILON) {
	    emcmotStatus->external_offsets_applied = 1;
	}
    }
        ...
        if ( !*(axis_data->eoffset_enable) ) {
            ...
            if (  last_eoffset_enable[axis_num]
               ...
               rtapi_print_msg(RTAPI_MSG_NONE,
                           "*** Axis %c External Offset = %.4f\n"
                           "*** External Offset disabled while NON-zero\n"
                           "*** To clear: re-enable & zero or use Machine-Off\n",
                           "XYZABCUVW"[axis_num],*(axis_data->external_offset) );
#else
               // as error message:
               reportError("Axis %c External Offset = %.4f\n"
                           "External Offset disabled while NON-zero\n"
                           "To clear: re-enable & zero or use Machine-Off",
                           "XYZABCUVW"[axis_num],*(axis_data->external_offset) );
#endif
